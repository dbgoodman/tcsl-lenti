...
cd4_barplot <- ggplot(cd4_60[[1]][donor %in% c(1,2) & 
                                  measurement==chosen_measure & 
                                  k562=='mkate_cd19'], 
              ...
              geom_text(aes(x=car, y=t_cross+5, group=t_cross, label=t_cross), 
                        ...
                        size=4) +
              geom_text(data=cd4_60[[2]][donor %in% c(1,2) & car!='none' & 
                                         measurement==chosen_measure & 
                                         k562=='mkate_cd19'], 
...
ggsave(paste0(output_dir, '/60pct_killing_hr_barplot_cd4.pdf'),
       cd4_barplot, width = 7, height = 4,
       units = 'in', dpi = 300)


cd8_barplot <- ggplot(cd8_60[[1]][measurement==chosen_measure & 
                                      k562=='mkate_cd19'], 
              ...
              geom_text(aes(x=car, y=t_cross+2, group=t_cross, label=t_cross), 
                        position = position_dodge(width=0.9),
                        size=2) +
              geom_text(data=cd8_60[[2]][car!='none' & 
                                         measurement==chosen_measure & 
                                         k562=='mkate_cd19'], 
...
ggsave(paste0(output_dir, '/60pct_killing_hr_barplot_cd8.pdf'),
       cd8_barplot, width = 7, height = 4,
       units = 'in', dpi = 300)
...
cd4_by_car <- ggplot(incucyte_dt[donor %in% c(1,2) & k562=='mkate_cd19' & !noise & t_type == 'cd4' & measurement == chosen_measure & car!='none'], 
...
ggsave(paste0(output_dir, '/killing_curves_by_car_cd4.pdf'),
       cd4_by_car, width = 7, height = 4,
       units = 'in', dpi = 300)



cd8_by_car <- ggplot(incucyte_dt[k562=='mkate_cd19' & !noise & t_type == 'cd8' & measurement == chosen_measure & car!='none'], 
              aes(x=Elapsed, y=mean_value_none, group=interaction(car, day_f,donor), fill=day_f)) + 
              geom_line(aes(color=day_f)) + 
              facet_grid(t_type+donor~car) + 
              ggtitle("CD8 - Incucyte K562 Killing by mKate Total Integrated Intensity (normalized to starting)") +
              theme_minimal(base_size=9) + geom_vline(xintercept=0) + geom_hline(yintercept=0) +
              scale_x_continuous(expand=c(0,0), limits=c(0,70)) +
              scale_y_continuous('mKate Total Integrated Intensity\n(normalized to growth without CAR)',
                limits=c(1e-2,1e1), trans=log10_trans()) +
              theme(plot.title = element_text(hjust = 0.5)) + 
...
ggsave(paste0(output_dir, '/killing_curves_by_car_cd8.pdf'),
       cd8_by_car, width = 7, height = 4,
       units = 'in', dpi = 300)
...
plot_set_time <- function(df, elapsed_hr){
  ...
  df_plot[, 
    day_rank := factor(paste(rank(mean_value_none), car, day, donor, t_type, sep= '|')), 
  ...
  plot <- ggplot(df_plot,
    ...
    geom_linerange(aes(
      ymin=mean_value_none-std_dev_value_none,
      ymax=mean_value_none+std_dev_value_none)) +
    geom_point(
      color='grey30', 
      shape=21, position=position_dodge(width=1),
      size=3) +
    facet_grid(donor ~ day_f, space='free', scales='free') +
    theme_minimal(base_size=9) +
    theme(
...
killing_plots_combined_cd8 <- plot_grid(
  plot_set_time(
...
killing_plots_combined_cd4 <- plot_grid(
  plot_set_time(
