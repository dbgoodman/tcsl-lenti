...
ctv_2_days <- make_ridge_plot_panel(
  ctv_fig_subset_dt[k562=='cd19+' & donor == 2 & day %in% c('d24', 'd33')], 'ctv2_t') +
    labs(x='CTV (AU)', y= 'CAR', title=paste0('Proliferation (donor 2)')) + 
...
ctv_gated_means_dt <- ctv_fig_subset_dt[, list(
  value_mean= mean(value), value_sd= sd(value)),
  by=c('car','k562','day','donor','variable','t_type')][,
    value_rel_day := value_mean-mean(value_mean[car != 'KLRG1']),
      by=c('k562','day','donor','variable','t_type')]

ctv_gated_means_subset <- ctv_gated_means_dt[variable=='ctv2_t' & k562 == 'cd19+']

max_val <- ctv_gated_means_subset[, max(abs(value_rel_day[car != 'KLRG1']))]
ctv_gated_means_subset[abs(value_rel_day) > max_val, value_rel_day := max_val * sign(value_rel_day)]
...
ctv_ridge_d2 <- make_ridge_plot_panel(
  ctv_fig_subset_dt[k562=='cd19+' & donor == 2 & day %in% c('d14', 'd33')], 'ctv2_t') +
    labs(x='CTV (AU)', y= 'CAR', title=paste0('Proliferation (donor 2)'))
...
ctv_gated_means_donormerge_subset <- ctv_gated_means_donormerge_dt[variable %in% c('ctv2_t') & car != 'KLRG1' & k562 == 'cd19+']

ggplot(ctv_gated_means_donormerge_subset,
    aes(fill=0-value_rel_day_mean, y=factor(car, levels=rev(levels(factor(car)))), x=day)) + 
  geom_tile() + 
  facet_grid(k562 ~ t_type + variable) + 
  scale_fill_viridis_c(
    'Mean proliferation, relative to mean',
...
vals <- as.matrix(
  dcast(
    ctv_gated_means_subset[variable == 'ctv2_t'],
    car + t_type ~ day + donor, 
    value.var='value_rel_day'))

wilcox.test(as.numeric(vals[7,3:10]), as.numeric(vals[9,3:10]), paired=T)
```
...
rm(list=c('ctv_dt','ctv_fig_dt','ctv_banff_dt','ctv_banff_subset_dt','ctv_gated_dt'))
...
save(list=ls(),
  file=file.path(data.output.dir, 'ctv_plot_data.Rdata'))
```
