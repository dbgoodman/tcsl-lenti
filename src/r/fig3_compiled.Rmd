...
ctv_gated_means_donormerge_subset <- ctv_gated_means_donormerge_dt[variable %in% c('ctv2_t') & k562 == 'cd19+']

max_val <- ctv_gated_means_donormerge_subset[, max(abs(value_rel_day_mean[car != 'KLRG1']))]
ctv_gated_means_donormerge_subset[abs(value_rel_day_mean) > max_val, value_rel_day_mean := max_val * sign(value_rel_day_mean)]
