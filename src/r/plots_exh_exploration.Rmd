...
ggsave(paste0(output_dir, '/exh_pd1_line_plot_subset.pdf'),
       pd1_line_plot, width = 3, height = 9,
       units = 'in', dpi = 300)

ridge_plot <- function(data_dt, variable_name, x_limits=c(400, 2500)) {

  plot <- ggplot(
    map_day_0(data_dt[variable == variable_name])) + 
...
plot_exh_combo <- function(data_dt, variable_name, limits=c()) {
  
  plot_group <- (
    ridge_plot(data_dt[k562 == 'cd19+' & t_type == 'cd4'], variable_name, x_limits=c(400, 3000)) | 
...
ridge_plot(exh_dt[k562 == 'cd19+' & t_type == 'cd8' &
        value > -500 & day < 33], 'pd1_t')
  
cd8_pd1_diff_ridges <- ggplot(
    map_day_0(exh_dt)[variable=='pd1_t' & k562 == 'cd19+' & t_type == 'cd8' &
        value > -500 & day < 33]) + 
    geom_density_ridges(aes(x=value, y=car, fill=car, alpha=0.8, color=car,
      height =..ndensity..)) +
    scale_fill_manual(values=c(car_colors, untrans='grey', zeta='black')) +
    facet_grid(day~t_type+donor, scales='free') + 
...
cd4_pd1_diff_ridges <- ggplot(
    map_day_0(exh_dt)[variable=='pd1_t' & k562 == 'cd19+' & t_type == 'cd4' &
        value > -500 & day < 33]) + 
    geom_density_ridges(aes(x=value, y=car, fill=car, alpha=0.8,
      height =..ndensity..)) +
    scale_fill_manual(values=c(car_colors, untrans='grey', zeta='black')) +
    facet_grid(day~t_type+donor, scales='free') + 
...
plot_grid(
  xy_hist_plot(
    df=map_day_0(exh_cast_dt)[k562 == 'cd19+' & t_type == 'cd4' & donor == 1],
    x_var='cd45ra_t',
    y_var='cd27_t',
    t_type='cd4'),
  xy_hist_plot(
