...
motif_table <- domain_annot[domain_names,
  on=c(mnemonic='uniprot'), nomatch=NA][
    source != 'ens_protvar'][,
      .N, by='orig_name']
data.table(real_names=row.names(unlist(domain_seqs), seq=unlist(domain_seqs)))
...
motif_heatmap <- ggplot(
    tidyr::complete(motif_annot_subset, motif_group, label)) + 
  geom_tile(aes(x=motif_group, y=factor(tip.label.clean, levels=tree$tip.label.clean), 
    fill=max_source), color='gray80') +
  scale_fill_manual(na.value = 'white',
    values=c(
      uniprot='darkorange',netphos='blue4',elm_curated='cornsilk4', elm_predict='cornsilk2')) +
  theme_bw() +
...
dendro_row <- axis_canvas(motif_heatmap, axis = "x") + 
    geom_segment(data = segment(dendro_data_row), 
        aes(x = x, y = y, xend = xend, yend = yend))
plot_motifheat <- insert_xaxis_grob(motif_heatmap, 
            dendro_col, grid::unit(0.2, "null"), position = "top")
ggdraw(plot_motifheat)
```