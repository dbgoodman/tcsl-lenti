...
# just KLRG1 comparison
ggplot(incucyte_dt[k562=='mkate_cd19' & !noise & t_type == 'cd8' & measurement == chosen_measure & car %in% c('CD28','KLRG1','zeta','none')][, 
    car := factor(car, levels=c('CD28','KLRG1','zeta','none'))], 
  aes(x=Elapsed, y=mean_value_norm, group=interaction(car, day_f,donor), fill=car)) + 
  geom_line(aes(color=car)) + 
  geom_ribbon(aes(ymin=mean_value_norm-std_error, ymax=mean_value_norm+std_error), alpha=0.3) +
  facet_grid(t_type+donor~day_f) + 
...
ggplot(incucyte_dt[k562=='mkate_cd19' & !noise & t_type == 'cd8' & measurement == chosen_measure & car %in% c('CD28','zeta','KLRG1')], 
  aes(x=Elapsed, y=mean_value_none, group=interaction(car, day_f,donor), fill=car)) + 
  geom_line(aes(color=car)) + 
  facet_grid(t_type+donor~day_f) + 
  ggtitle("Incucyte K562 Killing by mKate Total Integrated Intensity (normalized to starting)") +
...
## Banff style with correct colors

```{r}

source(here::here('r','fig_colors.R'))

car_colors <- c(car_colors, none='grey30')

car_set <- c('41BB','CD28','BAFF-R','KLRG1','TACI', 'none')

incucyte_dt_subset <- incucyte_dt[day %in% c(15, 22) &
  k562=='mkate_cd19' & !noise & t_type == 'cd4' & 
  measurement == chosen_measure & car %in% car_set 
  & donor == 2 ][, 
    car := factor(
...
incucyte_dt_subset <- incucyte_dt[day %in% c(0, 29) &
  k562=='mkate_cd19' & !noise & t_type == 'cd8' & 
  measurement == chosen_measure & car %in% car_set 
  & donor == 1 ][, 
    car := factor(
...
## Smoothed, not ribbon, klrg1 comparison, banff style 

```{r}

source(here::here('r','fig_colors.R'))

car_set <- c('CD28','KLRG1','zeta', 'none')

incucyte_dt_subset <- incucyte_dt[
  k562=='mkate_cd19' & !noise & t_type == 'cd8' & 
  measurement == chosen_measure & car %in% car_set 
  & donor == 1 & day %in% c(0, 8)]
...
cd8_banff <- ggplot(incucyte_dt_subset, 
    ...
    scale_x_continuous(expand=c(0,0), limits=c(0,65), breaks=c(0, 15, 30, 45)) +
    scale_y_continuous('mKate Total Integrated Intensity\n(normalized to starting)',
        limits=c(0.06,1e1), trans=log10_trans(), labels = scales::percent_format(accuracy = 1)) +
    theme(plot.title = element_text(hjust = 0.5)) + 
    scale_color_manual(values=c(car_colors, none='grey60')) + 
    scale_fill_manual(values=c(car_colors, none='grey60')) + 
    theme(panel.spacing = unit(0.5, "lines"))
...
plot_killing_cd4_log <- ggplot(incucyte_new_dt[k562=='mkate_cd19' & !noise & t_type == 'cd4' & measurement == chosen_measure & 
    car!='none'], 
  aes(x=Elapsed, y=mean_value_none_rollmean5, group=interaction(car, day_f,donor), fill=day_f)) + 
  geom_line(aes(color=car)) + 
  facet_grid(t_type+donor~day_f) + 
  ggtitle("CD4 - Incucyte K562 Killing by mKate Total Integrated Intensity (normalized to starting)") +
  ...
  scale_y_continuous('mKate Total Integrated Intensity\n(normalized to growth without CAR)',
    limits=c(1e-2,3), trans=log10_trans()) +
  theme(plot.title = element_text(hjust = 0.5)) + 
  scale_color_manual(values=car_colors) + 
  scale_fill_manual(values=car_colors) + 
  geom_hline(yintercept=1,
...
plot_killing_cd4_linear <- ggplot(incucyte_new_dt[k562=='mkate_cd19' & !noise & t_type == 'cd4' & measurement == chosen_measure & 
    car!='none'], 
...
plot_grid(plot_killing_cd4_log, plot_killing_cd4_linear, ncol=1)
...
plot_killing_cd8_linear <- ggplot(incucyte_new_dt[k562=='mkate_cd19' & !noise & t_type == 'cd8' & measurement == chosen_measure & 
    car!='none' & donor %in% c(1,2)], 
  aes(x=Elapsed, y=mean_value_none_rollmean10, group=interaction(car, day_f,donor), fill=day_f)) + 
  geom_line(aes(color=car)) + 
  facet_grid(t_type+donor~day_f) + 
...
plot_grid(plot_killing_cd8_log, plot_killing_cd8_linear, ncol=1)
```
## Save
...
save(list=c('incucyte_new_dt'),
  file=file.path(data.output.dir, 'incucyte_plot_data.Rdata'))
```
