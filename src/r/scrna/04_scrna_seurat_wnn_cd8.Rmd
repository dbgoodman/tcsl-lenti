...
SEURAT_RDS_CD8_WNN_FILE <- here::here(
    '..','..','scrnaseq_tcsl154',
    'rds','scrna.wnn.cd8.rds')
...
DimPlot(subset(scrna_cd8, subset=seurat_clusters %in% cluster_annotations$seurat_clusters), cols=cluster_cols, pt.size=2)
...
features <- c('IFNG','LTA','LTB','GNLY','GZMH','GZMA','PFN1','GZMB','GZMK','CCL5')
assay <- 'SCT'
split_name <- 'Subtype'
...
FeaturePlot(subset(scrna_cd8, subset=Subtype %in% names(cluster_cols)), features=paste(c('CD62L','CD45RO','CD30','CD49b','CD80','CD7','PD1','Tim3','IL2RA'),'adt',sep='.'), label=F)

FeaturePlot(subset(scrna_cd8, subset=Subtype %in% names(cluster_cols)), features=c('IL5','IL13','IFNG','CCL5','BATF3','IL7RA','GZMB','CD69','GATA3','CD52','PGK1','HSP90AA'), label=F)
...
# --- reticulate (below) wasn't working, did to.loom instead and from there to scanpy
# (https://satijalab.org/seurat/v3.0/conversion_vignette.html)

# Sys.setenv(RETICULATE_PYTHON = '/home/ec2-user/anaconda3/envs/scvi-env/bin/python3')
# #library(reticulate)
# reticulate::use_condaenv('scvi-env', required=T)
# library(reticulate)
# conda_python(envname = 'scvi-env', conda = "auto")
# use_python('~/anaconda3/envs/scvi-env/bin/python3')
# #conda_install(envname='scvi-env', packages=c('scanpy','scvi'))
# 
# sc <- import('scanpy', convert = FALSE)
# scvi <- import('scvi', convert = FALSE)
# scvi$settings$progress_bar_style = 'tqdm'

scrna.loom <- as.loom(readRDS(SEURAT_RDS_FILE), filename=here::here(
    '..','..','scrnaseq_tcsl154','rds','scrna.loom'))
    
    tools/testfiles/w-err-cmpd2.err
hdf5-1.12.0/hl/tools/testfiles/w-err-cmpd3.err


```

# map activated subsets onto 'all'
...
DefaultAssay(scrna_cd8_all) <- 'RNA'
scrna_cd8_all <- PercentageFeatureSet(scrna_cd8_all, pattern = "^MT-", col.name = "percent.mt")
scrna_cd8_all <- SCTransform(scrna_cd8_all, vars.to.regress = "percent.mt", verbose = FALSE)
scrna_cd8_all <- RunPCA(scrna_cd8_all)
...
#g2m.genes <- c(g2m.genes, 'HIST1H4C')
#https://satijalab.org/seurat/v3.1/cell_cycle_vignette.html
scrna_cd8_all <- CellCycleScoring(scrna_cd8_all, s.features = s.genes, g2m.features = g2m.genes, set.ident = TRUE)

scrna_cd8_all <- RunPCA(scrna_cd8_all, reduction.name = 'cc_pca')

# plot of cells by cell cycle, car, and stim
cc_plots <- plot_grid(
  DimPlot(scrna_cd8_all, reduction='cc_pca'),
  DimPlot(scrna_cd8_all, reduction='cc_pca', group='car'),
  DimPlot(scrna_cd8_all, reduction='cc_pca', group='k_type'))

cc_plots

# regress out the cell cycle scores and check PCA

scrna_cd8_all <- SCTransform(scrna_cd8_all, vars.to.regress = c("S.Score", "G2M.Score"))
scrna_cd8_all <- RunPCA(scrna_cd8_all, features = VariableFeatures(scrna_cd8_all), nfeatures.print = 10)

# phase analysis by car
ggplot(
  data.table(scrna_cd8_all@meta.data)[, .N, by=c('Phase','k_type','car')][,
    list(Phase, pct=N/sum(N)), by=c('k_type','car')]) + 
  geom_bar(aes(y=pct, fill=Phase, x=car), stat='identity', position='dodge') +
  facet_wrap(~k_type)
...
# do same for ADT
DefaultAssay(scrna_cd8_all) <- 'ADT'
scrna_cd8_all <- SCTransform(
  scrna_cd8_all, assay='ADT', new.assay.name = 'SCT_ADT', vars.to.regress = c("S.Score", "G2M.Score"))
scrna_cd8_all <- RunPCA(
  scrna_cd8_all, features = rownames(scrna_cd8_all[["SCT_ADT"]]), nfeatures.print = 10, reduction.name='adt_pca')

scrna_cd8_all <- FindMultiModalNeighbors(
  scrna_cd8_all, reduction.list = list("pca", "adt_pca"), 
  dims.list = list(1:30, 1:18), modality.weight.name = "RNA.weight"
)
...
scrna_cd8_all <- RunUMAP(scrna_cd8_all, nn.name = "weighted.nn", reduction.name = "wnn.umap", reduction.key = "wnnUMAP_")
scrna_cd8_all <- FindClusters(scrna_cd8_all, graph.name = "wsnn", algorithm = 3, resolution = cluster_resolution, verbose = FALSE)

p1 <- DimPlot(scrna_cd8_all, reduction = 'wnn.umap', label = TRUE, repel = TRUE, label.size = 2.5) + NoLegend()
p2 <- DimPlot(scrna_cd8_all, reduction = 'wnn.umap', group.by = 'car', label = TRUE, repel = TRUE, label.size = 2.5)
p3 <- DimPlot(scrna_cd8_all, reduction = 'wnn.umap', group.by = 'k_type', label = TRUE, repel = TRUE, label.size = 2.5) + NoLegend()
p4 <- DimPlot(scrna_cd8_all, reduction = 'wnn.umap', group.by = 'Phase', label = TRUE, repel = TRUE, label.size = 2.5) + NoLegend()
p1 + p3 + p4 + p2


car_clust_data <- data.table(
  scrna_cd8_all@meta.data)[, list(n_clust_car= .N), 
...
DimPlot(scrna_cd8_all, group.by='act_clust', label=T)

DimPlot(scrna_cd8_all, cols=activation_colors$fill[order(activation_colors$x)], label=T)


#associate activation clusters with all cells
act_clust_data <- data.table(scrna_cd8@meta.data, keep.rownames=T)[,
  list(rn, act_clust= seurat_clusters, act=Activation, sub=Subtype)][
    data.table(scrna_cd8_all@meta.data, keep.rownames=T), on=('rn')][,
      list(act_clust, act, sub)]

scrna_cd8_all@meta.data$act_clust <- act_clust_data$act_clust
scrna_cd8_all@meta.data$act <- act_clust_data$act
scrna_cd8_all@meta.data$sub <- act_clust_data$sub

DimPlot(scrna_cd8_all, group.by='act_clust')
DimPlot(scrna_cd8_all, group.by='sub', cols=cluster_cols)

DefaultAssay(scrna_cd8_all) <- 'SCT_ADT'
scrna_cd8_umap_plot_adt <- umap_plot(
  scrna_cd8_all, 
  plot_title_text="CD8, CD19+ ADT WNN", assay='SCT_ADT', cluster_name=cluster_name, reduction= 'wnn.umap')
scrna_cd8_umap_plot_adt$umap_plots
FeaturePlot(
  scrna_cd8_all, 
  features=paste(c('CD62L','CD45RO','CD30','CD137','HLA-ABC','CD49b','CD80','CD7','LAG3','Tim3','CD32','CTLA4'),'adt',sep='.'), label=T)

DefaultAssay(scrna_cd8_all) <- 'SCT'
scrna_cd8_umap_plot_rna <- umap_plot(
  scrna_cd8_all, 
  plot_title_text="CD8, CD19+ RNA WNN", assay='SCT', cluster_name=cluster_name, reduction= 'wnn.umap')
scrna_cd8_umap_plot_rna$umap_plots
FeaturePlot(
  scrna_cd8_all, 
  features=c('CD62L','CD45RO','CD30','CD137','HLA-ABC','CD49b','CD80','CD7','LAG3','Tim3','CD32','CTLA4'), label=T)

saveRDS(scrna_cd8_all, here::here(
    '..','..','scrnaseq_tcsl154',
    'rds','scrna.wnn.cd8.all.rds'))
...
### Clusters for act and inact

```{r}

#annotated_clusters
cluster_annotations <- fread(text="
act_clust   Activation    Subtype
0         INACT         REST
1         ACT           ACT-GLYC
2         INACT         UNTR
3         ACT           ACT-OXPHOS
