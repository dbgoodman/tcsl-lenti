...
pathway_enrichment <- function(obj_subset, set1, set2, ident='car', assay='RNA',
    logfc.thresh=0.25, min.pct=0.2, minGSSize=3, maxGSSize=100) {
  
  print(paste(paste(set1, collapse=','), 'vs', paste(set2, collapse=',')))
  
  organism = "org.Hs.eg.db"
  
  Idents(obj_subset) <- ident

  gene_data <- data.table(
    FindMarkers(
      obj_subset, assay=assay,
      ident.1 = set1, ident.2=set2,
  ...
  gse_pa_pos <- enrichPathway(gene=names(entrez_gene_list[entrez_gene_list > 0]),
    minGSSize = minGSSize, 
    maxGSSize = maxGSSize, 
    pvalueCutoff=0.05,
    readable=T)
  
  gse_pa_neg <- enrichPathway(gene=names(entrez_gene_list[entrez_gene_list < 0]),
    minGSSize = minGSSize, 
    maxGSSize = maxGSSize, 
    pvalueCutoff=0.05,
    readable=T)
  ...
  results <- list(
    ...
    gse_bp= gse_bp,
    gse_kegg= gse_kegg,
    gse_pa_pos= gse_pa_pos,
    gse_pa_neg= gse_pa_neg
  )
...
comp_car_volcano <- function(obj_subset, ident.1, ident.2, logfc.thresh=0.2, min.pct=0.1,
  ...
  return(list(markers=markers, plot=volcano_ggplot))
}
...
umap_subset <- function(this_subset, 
    ...
    save=T, save_plots=T, rerun=F, return_all=F) {
  ...
  # plots
  subset_umap_plot <- umap_plot(this_subset_umap, plot_title_text, assay, cluster_resolution)
  ...
  if (assay == 'RNA') {
    Idents(this_subset) <- 'car'
    a_sets = list(c('BAFF-R'), c('TACI'), c('CD40'), c('BAFF-R','TACI'), c('BAFF-R','TACI','CD40','4-1BB'), c('4-1BB'), c('BAFF-R','TACI','CD40','4-1BB','CD28'), c('CD28'))
    b_sets = list(c('CD28'), c('CD28','Zeta'), c('CD28','4-1BB','Zeta'), c('Zeta'), c('4-1BB'))
    volcano_plots = list()
    ...
    for (ident_a in a_sets) {
      ...
      for (ident_b in b_sets) {
        ...
        if (save_plots) {
          fn = paste0(plot_dir,'/',subset_name,'.',
            ident_a_name,'_VS_',ident_b_name,'.volcano.pdf')
          ggsave(fn, volcano_plots[[ident_a_name]][[ident_b_name]]$plot, width=8, height=8, units='in')
        }
      }
    }
  }
