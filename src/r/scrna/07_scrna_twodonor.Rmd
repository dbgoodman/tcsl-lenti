...
DimPlot(scrna.integrated, reduction = 'wnn.umap', group.by = 'subtype_154',
...
# get_cluster_enrich_plot(subset(scrna_cd8_all, Subtype != 'REMOVE'),
#   'Subtype', hide='K_only', facet_formula=formula(k_type~car))

# expr <- FetchData(scrna_cd8_all, c('Subtype','car')); get_cluster_enrich_plot(scrna_cd8_all[, which(expr$car != 'K_only' & expr$car != 'CD30' & expr$Subtype != 'REMOVE')],
#   'Subtype', facet_formula=formula(k_type~car))
...
scrna_all[['RNA']] <- int_cd8_all_subset@assays$RNA
scrna_all[['ADT']] <- int_cd8_all_subset@assays$ADT
...
scrna_cd8 <- AddModuleScore(scrna_cd8, assay='RNA', features=list('1'=glycolysis_genes),name='hallmark_glycolysis')
scrna_cd8 <- AddModuleScore(scrna_cd8, features=list('1'=Sheih_et_al_IRF),name='Sheih_et_al_IRF')

oxphos_gene_clust <- heatmap_clust_v_genes(
  scrna_cd8, 'seurat_clusters', assay='RNA',
  features=oxphos_genes, use_pct=T, use_slot='data')$plot
...
plot_cluster_pca <- function(seurat_obj, cluster_on='seurat_clusters',idents='donor') {
  
  obj.avg <- AverageExpression(seurat_obj,
    assay=c('SCT_INT','SCT_ADT_INT'), add.ident=idents, slot='scale.data')
  ...
  return(list(plot=pca_plot, metrics=projected.metrics, pca=pca.dt))
}

cd8_all_pca_plot <- plot_cluster_pca(subset(scrna_cd8_all, subset=(seurat_clusters != 29 & seurat_clusters != 26))
...
cluster_name <- 'Subty[e'
car_clust_pct <- data.table(scrna_cd8@meta.data[donor == 'TCSL154'])[
...
#### Clust Assignment Act Plots

```{r}

#mapping to old clusts:
cd8_all_old_map <- compare_clusters(scrna_cd8_all, 'seurat_clusters', 'subtype_154')

# best subtypes per cluster
cd8_all_old_map$clust_co_counts[subtype_154==best_b_in_a, list(seurat_clusters, subtype_154, pct_b_in_a, pct_a_in_b)][order(subtype_154)]
# best clusters per subtype
cd8_all_old_map$clust_co_counts[best_a_in_b==seurat_clusters, list(seurat_clusters, subtype_154, pct_b_in_a, pct_a_in_b)][order(subtype_154)]
#---
  
#mapping to new clusts:
cd8_all_new_map <- compare_clusters(scrna_cd8_all, 'seurat_clusters', 'Subtype')

# best subtypes per cluster
cd8_all_new_map$clust_co_counts[Subtype==best_b_in_a, list(seurat_clusters, Subtype, pct_b_in_a, pct_a_in_b)][order(Subtype)]
# best clusters per subtype
cd8_all_new_map$clust_co_counts[best_a_in_b==seurat_clusters, list(seurat_clusters, Subtype, pct_b_in_a, pct_a_in_b)][order(Subtype)]

#cd8 inact cluster adt markers
heatmap_clust_v_genes(subset(scrna_cd8_all, seurat_clusters %in% c(0,2,4,8,13,20,21,18,5,12,14,15)), 'seurat_clusters', assay='SCT_ADT_INT', features=rownames(scrna_cd8_all@assays$SCT_ADT_INT)[-2], use_slot='scale.data')

#module scores
scrna_cd8_all <- AddModuleScore(scrna_cd8_all, assay='RNA', features=list('1'=oxphos_genes),name='hallmark_oxphos')
scrna_cd8_all <- AddModuleScore(scrna_cd8_all, assay='RNA', features=list('1'=glycolysis_genes),name='hallmark_glycolysis')
scrna_cd8_all <- AddModuleScore(scrna_cd8_all, features=list('1'=Sheih_et_al_IRF),name='Sheih_et_al_IRF')
...
### CD8 inact cluster assignment
```{r}

# this took about 1.5 hrs:

# cd8_inact_var_features <- FindAllMarkers(subset(scrna_cd8_all, seurat_clusters %in% c(0,2,4,8,13,20,21,18,5,12,14,15)), assay='SCT_INT')
# 
# saveRDS(cd8_inact_var_features, here::here(
#   '..','..','scrnaseq_tcsl154',
#   'rds','scrna.wnn.int.CD8.all.inact_var_features.rds'))

cd8_inact_var_features <- readRDS(here::here(
  '..','..','scrnaseq_tcsl154',
  'rds','scrna.wnn.int.CD8.all.inact_var_features.rds')))

cd8_inact_var_plot <- heatmap_clust_v_genes(subset(scrna_cd8_all, seurat_clusters %in% c(0,2,4,8,13,20,21,18,5,12,14,15)), 'seurat_clusters', assay='SCT_INT', features=unique(data.table(cd8_inact_var_features)[, .SD[c(order(-avg_log2FC)[1:20])], by='cluster']$gene), use_slot='scale.data')
```

### TICA data

```{r}
#wget https://zenodo.org/record/4263972/files/TICAtlas_downsampled_1000.h5ad?download=1
#mv TICAtlas_downsampled_1000.h5ad\?download\=1 TICAtlas_downsampled_1000.h5ad
library(SeuratDisk)
Convert(here::here('..','data','TICAtlas_downsampled_1000.h5ad'), dest = "h5seurat", overwrite = TRUE)
scrna_TICAtlas <- LoadH5Seurat(here::here('..','data','TICAtlas_downsampled_1000.h5seurat'))
Idents(scrna_TICAtlas) <- scrna_TICAtlas@meta.data$cell_type
t_cell_types <- unique(grep('T cell', scrna_TICAtlas@meta.data$cell_type, value=T))
scrna_TICAtlas_t <- FindVariableFeatures(subset(scrna_TICAtlas, cell_type %in% t_cell_types))

TICA_markers <- data.table(
  FindAllMarkers(scrna_TICAtlas_t, 
  features=rownames(scrna_TICAtlas@assays$RNA@data), only.positive=T))

TICA_modules <- list()

for (cluster_i in levels(TICA_markers$cluster)) {
  TICA_modules[[cluster_i]] <- TICA_markers[cluster==cluster_i]$gene
}

scrna_cd8_all <- AddModuleScore(scrna_cd8_all, features=TICA_modules,name='TICA.')
```
...
# withr::with_makevars(
#   new=c(CXX14 = "g++", CXX14FLAGS= "-g -O3 $(LTO)",
...
ref <- ref[, grep('T cell',ref$label.fine)]
library(SingleR)
pred.hesc <- SingleR(test = as.SingleCellExperiment(scrna_cd8_all), ref = ref, assay.type.test=1,
    labels = ref$label.fine)
scrna_cd8_all[["SingleR.labels"]] <- pred.hesc$labels


#combine 2 & 4, 3 & 29, 1 is now IL7R
...
plot_grid(
  get_cluster_enrich_plot(scrna_cd4_all, 'seurat_clusters', hide='K_only', facet_formula=formula(k_type~car), bar_cutoff=0.01)$plot,
  DimPlot(scrna_cd4_all, reduction = 'wnn.umap', group.by = 'subtype_154', label = TRUE, repel = TRUE, label.size = 2.5) +
    DimPlot(scrna_cd4_all, reduction = 'wnn.umap', group.by = 'seurat_clusters', label = TRUE, repel = TRUE, label.size = 2.5),
  ncol=1)
...
scrna_all_plots <- (
   DimPlot(scrna_cd4_all, reduction = 'wnn.umap', label = TRUE, repel = TRUE, label.size = 2.5) + 
     NoLegend() +
   DimPlot(scrna_cd4_all, reduction = 'wnn.umap', group.by = 't_type', label = TRUE, repel = TRUE, label.size = 2.5) + 
     NoLegend() +
   DimPlot(scrna_cd4_all, reduction = 'wnn.umap', group.by = 'k_type', label = TRUE, repel = TRUE, label.size = 2.5) +
     NoLegend() +
   DimPlot(scrna_cd4_all, reduction = 'wnn.umap', group.by = 'Phase', label = TRUE, repel = TRUE, label.size = 2.5) + 
     NoLegend() +
   DimPlot(scrna_cd4_all, reduction = 'wnn.umap', group.by = 'donor', label = TRUE, repel = TRUE, label.size = 2.5) + 
     NoLegend() +
   DimPlot(scrna_cd4_all, reduction = 'wnn.umap', group.by = 'car', label = TRUE, repel = TRUE, label.size = 2.5) + 
     NoLegend() + 
   DimPlot(subset(crna_cd4_all, !is.na(subtype_154))), reduction = 'wnn.umap', group.by = 'subtype_154', label = TRUE, repel = TRUE, label.size = 2.5) + 
     NoLegend())


scrna_cd4_all <- AddModuleScore(scrna_cd4_all, features=list('1'=Nicolet_et_al_CD29core),name='Nicolet_et_al_CD29core')
scrna_cd4_all <- AddModuleScore(scrna_cd4_all, features=list('1'=Sheih_et_al_IRF),name='Sheih_et_al_IRF')
scrna_cd4_all <- AddModuleScore(scrna_cd4_all, assay='RNA', features=list('1'=glycolysis_genes),name='hallmark_glyolysis')
scrna_cd4_all <- AddModuleScore(scrna_cd4_all, assay='RNA', features=list('1'=oxphos_genes),name='hallmark_oxphos')
...
cd4_adt_features = c(
  'CD7.adt','CD62L.adt','IL7R.adt','CD29.adt','CD27.adt','CD28.adt',
  'CD45RA.adt','CD45.RO.adt','IL2RA.adt','Tim3.adt','OX40.adt','LAG3.adt')

FeaturePlot(scrna_cd4_all, features=cd4_adt_features)
...
ref <- ref[, grep('T cell',ref$label.fine)]
library(SingleR)
pred.hesc <- SingleR(test = as.SingleCellExperiment(scrna_cd4_all), ref = ref, assay.type.test=1,
    labels = ref$label.fine)
scrna_cd4_all[["SingleR.labels"]] <- pred.hesc$labels

cd4_avgs <- AverageExpression(
  scrna_cd4_all, assays=c('SCT_ADT_INT','SCT_INT'),
  group.by='seurat_clusters')

dist <- dist(t(rbind(cd4_avgs$SCT_ADT_INT, cd4_avgs$SCT_INT)[,c(0:8,10:19,21:28)]), diag=TRUE)
hc <- hclust(dist)
plot(hc)
...
# adt maps for cd4_all

#http://www.gsea-msigdb.org/gsea/msigdb/download_geneset.jsp?geneSetName=HALLMARK_PI3K_AKT_MTOR_SIGNALING&fileType=txt
# copy pathways from one to another:
# for (pathway in gsub('.$','',grep('1$',names(scrna_cd4_all@meta.data), value=T))) AddModuleScore(scrna_all, features=list('1'=get_geneset(pathway)), name=pathway)
#map from cd4_all to all
...
scrna_all <- AddModuleScore(scrna_all, features=list('1'=Nicolet_et_al_CD29core),name='Nicolet_et_al_CD29core')
scrna_all <- AddModuleScore(scrna_all, features=list('1'=Sheih_et_al_IRF),name='Sheih_et_al_IRF')




scrna_all <- map_clusters(scrna_cd4_all, scrna_all, 'seurat_clusters', 'all_clust_cd4')
cd4_154_clusters <- compare_clusters(scrna_all[, scrna_all[['t_type']]=='CD4'], 'seurat_clusters', 'subtype_154_cd4')
cd4_154_clusters$clust_co_counts[, n_clust := sum(n_clust_car[!is.na(subtype_154_cd4)]), by=seurat_clusters]
cd4_154_clusters$clust_co_counts[n_clust > 100 & subtype_154_cd4 == best_b_in_a]

### sct scores for cd29 core etc
# sct scores look better than raw rna (not scaled)
scrna_cdr_all <- AddModuleScore(scrna_cdr_all, features=list('1'=Nicolet_et_al_CD29core),name='Nicolet_et_al_CD29core_sct')
scrna_cd8_all <- AddModuleScore(scrna_cd8_all, features=list('1'=Nicolet_et_al_CD29core),name='Nicolet_et_al_CD29core_sct')
plot_grid(
  VlnPlot(scrna_cd4_all, features=c('Nicolet_et_al_CD29core_sct1'), group.by='car', y.max=5, pt.size=0) + geom_boxplot(width=0.1, fill="white"),
  VlnPlot(scrna_cd8_all, features=c('Nicolet_et_al_CD29core_sct1'), group.by='car', y.max=5, pt.size=0) + geom_boxplot(width=0.1, fill="white"))
plot_grid(
  VlnPlot(scrna_cd4_all, features=c('Nicolet_et_al_CD29core_sct1'), group.by='Subtype', y.max=5, pt.size=0) + geom_boxplot(width=0.1, fill="white"),
  VlnPlot(scrna_cd8_all, features=c('Nicolet_et_al_CD29core_sct1'), group.by='Subtype', y.max=5, pt.size=0) + geom_boxplot(width=0.1, fill="white"))
```