...
if (rerun_find_markers) {
  ...
  cd4_rna_mast_markers <- loadRDS(cd4_marker_genes.rds.file)
  cd8_rna_mast_markers <- loadRDS(cd8_marker_genes.rds.file)
}
...
plots$matched_clusters <- ggplot(diff_gene_n) + 
  geom_tile(aes(
    x=factor(subtype_1, levels=cd4_order), 
    y=factor(subtype_2, levels=cd8_order), 
    fill=N/top_n)) + 
  geom_tile(aes(
    x=factor(subtype_1, levels=cd4_order), 
    y=factor(subtype_2, levels=cd8_order),
