...
SaveH5Seurat(scrna_cd4_all, all.h5.file, overwrite=T)
```
...
make_gene_heatmap <- function(seurat_obj, subtype_order, max_genes=100) {
  ...
  #reorder
  new_gene_order <- c()
  for (group_i in na.omit(unique(melted_genes$best_subtype))) {
  ...
  de_gene_heatmap <- ggplot(melted_genes[gene %in% best_genes]) + 
    geom_raster(aes(x=interaction(donor,subtype), y=gene, fill=zscore)) +
    scale_x_discrete(expand=c(0,0)) + 
    scale_y_discrete(expand=c(0,0)) +
    geom_vline(color='white', xintercept=0.5+c(1:n_lines-1)*2) +
    facet_grid(best_subtype~., scales='free', space='free') +
...
cd8_gene_heatmap <- make_gene_heatmap(prune_cells(scrna_cd8_all), cd8_order, max_genes=100)
cd4_gene_heatmap <- make_gene_heatmap(prune_cells(scrna_cd4_all), cd4_order, max_genes=100)
