...
plots$subtypes <- ggplot(umap_dt[sample(.N)], aes(x=wnnUMAP_1, y=wnnUMAP_2, color=Subtype)) +
  rasterise(geom_point(size=0.1, alpha=0.25), dpi=500) + 
  coord_fixed() +
  geom_label_repel(
    data=labels_dt,
    aes(x=wnnUMAP_1, y=wnnUMAP_2, label=subtype_lbl),
    fill= alpha(c("white"),0.75), force=0.5, size=3) + 
...
plots$per_phase <- ggplot(
  ...
  rasterise(geom_point(size=inset_umap_point_size*0.01, alpha=0.05), dpi=500) + 
  coord_fixed() +
...
plots$per_donor_sep <- ggplot(
    umap_dt[sample(.N)], aes(x=wnnUMAP_1, y=wnnUMAP_2, 
      color=factor(donor, labels=c('A','B')))) +
  rasterise(geom_point(
    data=copy(umap_dt)[, 'donor' := NULL],
    size=inset_umap_point_size*0.01, color='grey90'), dpi=500) +
  rasterise(geom_point(size=inset_umap_point_size*0.01, alpha=0.05), dpi=500) + 
  scale_color_discrete('Donor') +
  coord_fixed() +
  facet_wrap(~donor) +
...
plots$activation <- ggplot(
  ...
  scale_color_distiller(
    ...
    breaks=c(-5,-3,0,3,5),
    labels=rev(c('32:1','8:1','1:1','1:8','1:32'))) +
  coord_fixed() +
  theme_minimal() + 
  theme(
    axis.text=element_blank(),
...
#activation v2
plots$activation2 <- ggplot(
    umap_dt[sample(.N)], aes(x=wnnUMAP_1, y=wnnUMAP_2, 
      color=factor(k_type, levels=c('baseline','cd19+','bead_stim'), labels=c('Unstim','CD19 Stim', 'Untr\n+CD3/CD28 Beads')))) +
  rasterise(geom_point(
    data=copy(umap_dt)[, 'donor' := NULL],
    size=inset_umap_point_size*0.01, color='grey90'), dpi=500) +
  rasterise(geom_point(size=inset_umap_point_size*0.01, alpha=0.2), dpi=500) + 
  scale_color_manual('Stimulation',
    values=c(
      brewer.pal(11,'PuOr')[c(9,3)],
      brewer.pal(11,'PiYG')[c(9)]),
      ) +
  coord_fixed() +
  theme_minimal() + 
...
plots$combined_umap_vert_v2 <- ((plots$subtypes + theme(legend.position='none')) / 
  (plots$per_phase | plots$per_donor_sep | plots$activation) + plot_layout(widths=c(1,2,1))
...
plots$per_car_umap <- ggplot(sampled_umap, aes(x=wnnUMAP_1, y=wnnUMAP_2, color=
  ...
  rasterise(geom_point(
    ...
    size=0.1, color='grey90'), dpi=200) + 
  rasterise(geom_point(size=0.05, alpha=0.1), dpi=200) +
  coord_fixed() +
...
interpolate_score <- function(umap_data, resolution, score_name, plot=T, pal_name) {
  ...
  data[, scaled := scales::rescale(scaled, to=c(0,1)), by=c('t_type')]
  ...
  if(plot) {
    return(ggplot(data, aes(x=wnnUMAP_1, y=wnnUMAP_2)) +
      rasterise(geom_point(size=0.001, color='grey50', alpha=0.01), dpi=200) +
...
make_minidot_plot <- function(umap_data, module_name, rescale_t_types=F, reorder_x=F) {
  
  if (rescale_t_types) {
    umap_data <- umap_data[, score := scale(get(module_name)), by=c('t_type')]
  } else {
    umap_data <- umap_data[, score := scale(get(module_name))]
  }
  
  umap_means <- umap_data[, 
  ...
  if (reorder_x) {
    type_ranks <- umap_means[, list(mean(-score)), by='t_subtype'][, t_subtype[order(V1)]]
    umap_means[, t_subtype := factor(t_subtype, levels=type_ranks)]
  }
  
  plot <- ggplot(umap_means, aes(
      x=t_subtype, y=score, fill=Subtype, 
      color=Subtype, group=interaction(donor,t_subtype))) +
    geom_linerange(
...
make_minidot_plot_car <- function(umap_data, module_name, run_test=F, rescale_t_types=F, reorder_x=F) {
  ...
  umap_means <- umap_means[, t_car := interaction(t_type, car)]
  
  if (reorder_x) {
    car_ranks <- umap_means[, list(mean(-score)), by=c('t_car')][, t_car[order(V1)]]
    umap_means[, t_car := factor(t_car, levels=car_ranks)]
  }
  
  plot <- ggplot(umap_means, aes(
      x=t_car, y=score, fill=car, 
...
```{r}
car_pct_dotplot <- function(
    car_metadata, subtype_order, max_size=15, flip=T, legend.pos='right', car_text_relsize=1.2) {
  ...
  pct_dotplot <- ggplot(car_clust_pct_act) + 
    geom_point(aes(
        ...
        y=factor(car, umap_car_order, gsub('Untransduced','Untrans.', umap_car_order)),
...
plots$car_dotplot_unstim_vert <- (
  ((car_pct_dotplot(
      data.table(scrna_cd4_all@meta.data)[
        k_type == 'baseline' & !is.na(Subtype) & car != 'K_only' & !grepl('STIM',Subtype) &
        car != 'CD30' & Subtype != 'REMOVE' &
        Subtype != 'NA'], cd4_order, max_size=9, legend.pos='none', car_text_relsize=0.9) +
      ggtitle('CD4 Baseline') + 
      theme(legend.position='none')) |
     (car_pct_dotplot(
      data.table(scrna_cd8_all@meta.data)[
        k_type == 'baseline' & !is.na(Subtype) & car != 'K_only' & !grepl('STIM',Subtype) &
        car != 'CD30' & Subtype != 'REMOVE' &
        Subtype != 'NA'], cd8_order, max_size=9, legend.pos='none', car_text_relsize=0.9) +
      ggtitle('CD8 Baseline'))) & 
      theme(plot.title.position="plot")
  ) + 
  plot_layout(widths=c(1,10,10)) & 
  theme(plot.margin=unit(c(0,0,0,0),'points'))
...
diff_gene_n[, subtype_1 := factor(subtype_1,
...
plots$matched_clusters <- ggplot(diff_gene_n,
  ...
  geom_tile(aes(
    fill=N/top_n)) + 
  geom_tile(aes(
    color=(as.character(subtype_1) == as.character(subtype_2) &
      subtype_1 != 'Baseline')), fill=NA, size=0.5) + 
...
### Assemble individual components of gene heatmaps
...
plots$cd4_gene_heatmap_boxed <- make_gene_heatmap(
  cd4_pruned, cd4_order, 
  max_genes=100, label_genes=cd4_genes, rescale=F, 
...
umap_protein_ranges <- list(
  'CD62L.adt'=c(0,4),
  'CD45RA.adt'=c(0,3),
  'CD45RO.adt'=c(-0.5,1.75),
  'HLA-ABC.adt'=c(0,2.5),
  'CD49b.adt'=c(0,2.5),
  'IL2RA.adt'=c(0,8))

umap_range_dt <- data.table(
  variable=names(umap_protein_ranges),
  var.min=sapply(umap_protein_ranges, function (x) x[1]),
  var.max=sapply(umap_protein_ranges, function (x) x[2]))

prev_assay <- DefaultAssay(scrna_all)
DefaultAssay(scrna_all) <- 'SCT_ADT_INT'
citeseq_data <- data.table(
  FetchData(
    scrna_all,
    names(umap_protein_ranges),
    slot='data'),
  keep.rownames=T)
DefaultAssay(scrna_all) <- prev_assay
setnames(citeseq_data, 'rn','X_index')
citeseq_umap <- citeseq_data[umap_dt, on='X_index']
csu_melt <- melt(citeseq_umap, measure.vars=paste0(umap_proteins,'.adt'))

csu_melt <- csu_melt[umap_range_dt, on='variable'][, 
  value.scaled := scales::rescale(
    scales::oob_squish(value, range=c(var.min[1], var.max[1]))),
  by='variable']

var_order <- c('CD62L','CD45RA','CD45RO','CD49b','HLA-ABC','IL2RA')
csu_melt <- csu_melt[,
  variable := factor(variable, levels=paste0(var_order,'.adt'), labels=var_order)]

plots$citeseq_umaps <- ggplot(csu_melt,
    aes(x=wnnUMAP_1, y=wnnUMAP_2, color=value.scaled)) +
  rasterise(geom_point(size=inset_umap_point_size, alpha=0.5), dpi=500) + 
  scale_color_distiller('Protein expression (CITE-Seq)',
    palette='RdPu', direction=1, limit=c(0,1),
    breaks=c(0,1),
    labels=c('min','max')) +
  coord_fixed() +
  facet_wrap(~variable) +
...
umap_rna_range_dt <- data.table(
  variable=names(umap_rna_ranges),
  var.min=sapply(umap_rna_ranges, function (x) x[1]),
  var.max=sapply(umap_rna_ranges, function (x) x[2]))

prev_assay <- DefaultAssay(scrna_all)
DefaultAssay(scrna_all) <- 'SCT_INT'
umap_rna_data <- data.table(
  FetchData(
    scrna_all,
    names(umap_rna_ranges),
    slot='data'),
  keep.rownames=T)
DefaultAssay(scrna_all) <- prev_assay
setnames(umap_rna_data, 'rn','X_index')
sct_rna_umap <- umap_rna_data[umap_dt, on='X_index']
csu_melt <- melt(sct_rna_umap, measure.vars=names(umap_rna_ranges))

csu_melt <- csu_melt[umap_rna_range_dt, on='variable'][, 
  value.scaled := scales::rescale(
    scales::oob_squish(value, range=c(var.min[1], var.max[1]))),
  by='variable']

var_order <-  names(umap_rna_ranges)
csu_melt <- csu_melt[,
  variable := factor(variable, levels=var_order)]

plots$rna_umaps <- ggplot(csu_melt,
    aes(x=wnnUMAP_1, y=wnnUMAP_2, color=value.scaled)) +
  rasterise(geom_point(size=inset_umap_point_size, alpha=0.5), dpi=100) + 
  scale_color_distiller('RNA expression (SCTransform)',
    palette='RdPu', direction=1, limit=c(0,1),
    breaks=c(0,1),
    labels=c('min','max')) +
  coord_fixed() +
...
plots$rna_umaps_rect <- ggplot(csu_melt,
    aes(x=wnnUMAP_1, y=wnnUMAP_2, color=value.scaled)) +
  rasterise(geom_point(size=inset_umap_point_size, alpha=0.5), dpi=100) + 
  scale_color_distiller('RNA expression (SCTransform)',
    palette='RdPu', direction=1, limit=c(0,1),
...
gsea.pathways.file <- here::here(
     '..','..','scrnaseq_tcsl154',
     'rds',
     paste0('gsea_pathways.Rdata'))
...
if (rerun_geneset_scores) {
  ...
  save(list=c(
    'cd8_gsea_scores_rna',
    'cd8_all_gsea_scores_rna',
    'cd4_gsea_scores_rna',
    'cd4_all_gsea_scores_rna'),
    file=gsea.pathways.file)
} else {
  if (!exists('cd8_gsea_scores_rna'))
    load(gsea.pathways.file)
  cd8_pruned[[names(cd8_gsea_scores_rna)]] <- cd8_gsea_scores_rna
  cd4_pruned[[names(cd4_gsea_scores_rna)]] <- cd4_gsea_scores_rna
  scrna_cd8_all[[names(cd8_all_gsea_scores_rna)]] <- cd8_all_gsea_scores_rna
  scrna_cd4_all[[names(cd4_all_gsea_scores_rna)]] <- cd4_all_gsea_scores_rna
...
pathway_list <- list(
  ...
  'Cell Adhesion'='KEGG_CELL_ADHESION_MOLECULES_CAMS',
...
innateness_dt <- fread(here::here('..','data','gutierrez_arcelus_et_al_2019-sup_dat_3.csv'))

innateness_sig_dt <- innateness_dt[
  CalledSignificant==1 &
  GENE_NAME %in% scrna_all@assays$SCT_INT@var.features & 
  GENE_NAME != 'AKR1C3']

innateness_scrna_dt <- data.table(FetchData(prune_cells(scrna_all),
    vars=c(names(scrna_all@meta.data), innateness_sig_dt$GENE_NAME),
    slot='scale.data'))

innateness_scrna_dt$innateness <- scale(
  as.matrix(innateness_scrna_dt[, innateness_sig_dt$GENE_NAME, with=F]) %*% 
    innateness_sig_dt$`InnatenessLevel_(Beta)`)

# add wnn umap
innateness_scrna_dt <- umap_dt[, list(cell, wnnUMAP_1, wnnUMAP_2)][innateness_scrna_dt, on='cell']

innateness_scrna_dt[, t_subtype := paste(t_type, Subtype, sep='.')]

innateness_boxplots <- (
  (ggplot(innateness_scrna_dt) +
    geom_boxplot(aes(x=Subtype, y=innateness, color=Subtype, fill=Subtype), alpha=0.5) + 
    ...
    scale_fill_manual(values=all_colors)) | 
  ggplot(innateness_scrna_dt) +
    geom_boxplot(aes(x=car, y=innateness, color=car, fill=car), alpha=0.5) + 
    facet_grid(t_type~., scale='free') +
    theme_minimal() +
    coord_flip() + 
    scale_color_manual(values=car_colors_umap) + 
...
innateness_umap <- ggplot(
    innateness_scrna_dt[sample(.N)], aes(x=wnnUMAP_1, y=wnnUMAP_2, 
      color=innateness)) +
  rasterise(geom_point(size=inset_umap_point_size, alpha=0.5), dpi=300) + 
  scale_color_distiller('Lymphocyte Innateness\nZ-score\n(Gutierrez-Arcelus et al.)', palette='RdYlGn', direction=1, limits=c(-2,2), oob=scales::squish) +
  coord_fixed() +
  theme_minimal() + 
  guides(color = guide_colorbar(override.aes = list(size = 5, alpha=1), title.position='top', barheight = unit(0.4, 'line'))) +
  theme(
...
# innateness_minidot_plots <- (
#    / (
#   make_minidot_plot_car(
#     innateness_scrna_dt[sample(.N)], 'innateness', rescale_t_types = T, reorder_x=T) +
#     labs(y='CAR z-score') +
...
### Combine All Paper UMAP plot sets
...
plots$paper_correlation_umap <- (
  ...
  (interpolate_score(umap_dt, 500, 'Nicolet_et_al_CD29core1', pal_name='Greys') +
    ggtitle('Nicolet et al. 2020', 
      subtitle='CD29+ Cytotoxic signature,\npredicts survival in Melanoma')) |
  (interpolate_score(innateness_scrna_dt, 500, 'innateness', pal_name='Greys') + 
    ggtitle('Gutierrez-Arcelus et al. 2019', subtitle='Lymphocyte Innateness'))) &
  theme(
    legend.position='none',
    plot.margin = unit(c(5.5, 5.5, 0, 5.5), "points"))

cluster_zscore_plots <- (((
  make_minidot_plot(umap_dt, 'Sheih_et_al_IRF1', rescale_t_types = T, reorder_x=T) + 
    labs(y='cluster z-score') + 
    theme(axis.title.y=element_text(angle = 90, vjust = 0.5, hjust=0.5))) | 
  make_minidot_plot(umap_dt, 'Nicolet_et_al_CD29core1', rescale_t_types = T, reorder_x=T) |
  make_minidot_plot(innateness_scrna_dt[sample(.N)], 'innateness', 
    rescale_t_types = T, reorder_x=T)))

car_zscore_plots <- (((
  make_minidot_plot_car(umap_nocd30_dt, 'Sheih_et_al_IRF1', rescale_t_types = T, reorder_x=T) + 
    labs(y='CAR z-score') + 
    theme(axis.title.y=element_text(angle = 90, vjust = 0.5, hjust=0.5))) | 
  make_minidot_plot_car(umap_nocd30_dt, 'Nicolet_et_al_CD29core1', rescale_t_types = T, reorder_x=T) |
  make_minidot_plot_car(innateness_scrna_dt[sample(.N)], 'innateness', 
    rescale_t_types = T, reorder_x=T)))
...
paper_umaps_vert <- ((
  interpolate_score(umap_dt, 500, 'Sheih_et_al_IRF1', pal_name='Greys') + 
    ggtitle('Clonotypes enriched after\nengraftment in CAR patients', subtitle='Sheih et al. 2020')) / 
  (interpolate_score(umap_dt, 500, 'Nicolet_et_al_CD29core1', pal_name='Greys') +
    ggtitle('CD29+ Cytotoxic signature,\npredicts survival in Melanoma', subtitle='Nicolet et al. 2020')) /
  (interpolate_score(innateness_scrna_dt, 500, 'innateness', pal_name='Greys') + 
    ggtitle('Lymphocyte Innateness', subtitle='Gutierrez-Arcelus et al. 2019'))) &
  theme(
...
combined_zscores <- function(datatable, var, datatable2=NULL) {
  if (is.null(datatable2)) datatable2 <- datatable
  (make_minidot_plot(datatable, var, rescale_t_types = T, reorder_x=T) + 
    theme(axis.title.y=element_text(angle = 90, vjust = 0.5, hjust=0.5)) +
      labs(y='z-score')) |  (
  make_minidot_plot_car(datatable2, var, rescale_t_types = T, reorder_x=T) + 
    theme(axis.title.y=element_blank())) 
}

paper_zscores_vert <- (combined_zscores(umap_dt, 'Sheih_et_al_IRF1', umap_nocd30_dt) / 
  combined_zscores(umap_dt, 'Nicolet_et_al_CD29core1', umap_nocd30_dt) / 
  combined_zscores(innateness_scrna_dt, 'innateness'))

plots$paper_umaps_combined_vert <- (paper_umaps_vert | paper_zscores_vert) + plot_layout(widths=c(1,2.3))
...
#width=1280&height=1366&scale=1
plots$fig_assembled_6 <- plot_grid(
  plot_grid(
    #A
    ((plots$subtypes + theme(legend.position='none')) / 
      (plots$per_phase | plots$per_donor_sep | plots$activation2) + 
        plot_layout(widths=c(1,4,1))) + plot_layout(heights=c(5,1)),
    #C
    plots$car_dotplot_horiz,
    #D,E
    plot_grid(
      plots$citeseq_umaps,
      plots$combined_tile_plot,
      ncol=2,
      rel_widths=c(1.2,1),
      labels=c('D','E')
    ),
  ...
  plot_grid(
    plots$combined_gene_heatmap_boxed,
    plots$paper_umaps_combined_vert,
    ncol=1,
...
scaling_factor <- 1.6
ggsave(here::here(
  '..','figs','compiled','fig5.pdf'),
  plots$fig_assembled_6, device='pdf',
...
rna_by_subtype_and_car <- function(
    rna_name,
    separate_by_donor=F,
    slot='counts',
    assay='RNA',
    rescale_data=F) {
  orig_assay <- DefaultAssay(scrna_all)
  DefaultAssay(scrna_all) <- assay
  rna_data <- data.table(FetchData(prune_cells(scrna_all),
    vars=c(names(scrna_all@meta.data), rna_name), slot=slot))
  
  stopifnot(names(rna_data)[length(names(rna_data))] == rna_name)
  ...
  y_axis_name <- 'RNA expression'
  if (slot == 'counts') y_axis_name <- 'UMIs per cell'
  if (slot == 'scale.data') y_axis_name <- 'z-score'
  
  if (slot == 'scale.data' & rescale_data) {
    rna_data[, RNA_count := scale(RNA_count), by=c('k_type','donor')]
  }
    
  by_subset <- ggplot(rna_data,
      aes_string(x='Subtype', y='RNA_count', color='Subtype', shape=interact,
    ...
    theme_minimal() +
    coord_flip() +
    labs(title=paste0(rna_name,' Expression by Subtype'), y=y_axis_name) +
    theme(
      panel.background=element_rect(fill=NA, color='grey50'),
      panel.grid=element_blank())
  
  by_car <- ggplot(rna_data, 
      aes_string(x='car', y='RNA_count', color='car', shape=interact,
  ...
  DefaultAssay(scrna_all) <- orig_assay
  
  if (slot == 'scale.data') {
    by_car <- by_car + geom_hline(yintercept=0, linetype=3, color='grey30', size=0.6)
    by_subset <- by_subset + geom_hline(yintercept=0, linetype=3, color='grey30', size=0.6)
  }
...
supp_S5_top <- (
  plots$per_car_umap /
  plot_grid(
    plots$matched_clusters,
    plots$car_dotplot_unstim_vert,
    ncol=2, nrow=1, rel_widths=c(1,2.5))
  ) + plot_layout(heights=c(1.2,1))
scaling_factor <- 1.6

supp_S5_donors <- plot_grid(
  plots$cd4_pct_barplots & theme(legend.position='none', axis.title.x=element_blank()),
  plots$cd8_pct_barplots, ncol=1, nrow=2, rel_heights=c(1,1.3))

supp_S5 <- plot_grid(
  supp_S5_top,
  plot_grid(
    (supp_S5_donors | as_grob(plots$combined_adt_plot + theme(legend.position='bottom'))) + 
...
supp_S5 <- plot_grid(
  ...
  plot_grid(
    plot_grid(
      plot_grid(
        NULL,
        plots$car_dotplot_unstim + theme(plot.margin=margin(0,0,0,10, unit='pt')),
        NULL,
        plots$matched_clusters + 
          labs(fill='% Overlap of top 100 DE genes') +
          theme(
            legend.position='top',
            legend.key.height = unit(0.4, 'lines')),
        NULL,
        nrow=1,
        rel_widths=c(0.05,1.1,0.05,0.9,0.10)),
      supp_S5_donors,
      ncol=1,
      rel_heights=c(1.2,2)),
    plots$combined_adt_plot + 
      theme(
        legend.position='bottom',
        legend.key.height = unit(0.4, 'lines')),
    ncol=2,
  ...
  plots$rna_umaps,
  nrow=3,
  rel_heights=c(1,3,0.8)
)

supp_S5_v2 <- plot_grid(
  ...
  plot_grid(
    ...
    NULL,
    nrow=1,
    rel_widths=c(0.05,1.25,0.25,1.5,0.25)),
  ...
  plot_grid(
    ...
    NULL,
    nrow=1,
    rel_widths=c(0.05,0.8,0.05,2,0.10)),
  
  #F
  plots$rna_umaps,
  nrow=4,
  rel_heights=c(1,1,1.9,0.8))

ggsave(here::here(
  '..','figs','compiled','figS5.pdf'),
  supp_S5_v2, device='pdf',
  width=8.5*scaling_factor,
  height=11*scaling_factor,
  units=c('in'),
  dpi=300)
```
