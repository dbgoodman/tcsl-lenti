...
plot_pca_cars <- function(obj_subset, title, axes=c('car','k_type'), min_pct_exp=10) {
  pca_data <- plot_pca(obj_subset, axes, min_pct_exp)
  plot_grid(
    pca_data$pca_plot + theme_minimal() + labs(title=title),
    plot_gene_assoc_bar(pca_data, 'PC1', n_genes=10),
    plot_gene_assoc_bar(pca_data, 'PC2', n_genes=10),
    ncol=3, nrow=1,
    rel_widths=c(5, 1, 1))
}
...
all_plots_for_subset <- function(subset, assay, ) {
...
umap_subset <- function(this_subset, 
  ...
  car_pca_plot <- plot_pca_cars(this_subset)
...
stimulatory_cars <- c('4-1BB','BAFF-R','CD40','TACI','CD28')
...
if (!reload_data && !('umap_plots' %in% ls()) )
{
  ...
  subset_names= c('CD4','CD8','CD19','CD4.CD19','CD8.CD19','baseline','CD4.baseline', 'CD8.baseline',
