...
plot_pca <- function(obj_subset, axes=c('car','k_type'), min_pct_exp = 10, assay='RNA') {
...
plot_pca_cars <- function(obj_subset, title, axes=c('car','k_type'), min_pct_exp=10, assay='RNA') {
  pca_data <- plot_pca(obj_subset, axes, min_pct_exp, assay)
  return(plot_grid(
...
umap_subset <- function(this_subset, 
  ...
  car_pca_plot <- plot_pca_cars(this_subset, title=plot_title_text, axes=c('car','k_type','CD4v8'))
  ...
  if (assay == 'RNA') {
    ...
    b_sets = list(c('CD28'), c('CD28','Zeta'), c('CD28','4-1BB','Zeta'))
    volcano_plots = list()
    
    for (ident_a in a_sets) {
      volcano_plots[[ident_a]] = list()
...
if (!reload_data && !('umap_plots' %in% ls()) )
{
  ...
  #     by='subset_name']
  umap_plots[subset_fxn_num >=10, RNA := list(list(
