...
ggplot(incucyte_dt[k562=='mkate_cd19' & !noise & t_type == 'cd4' & measurement == chosen_measure & car!='none' & Elapsed==24], 
  aes(x=car, y=mean_value_none, group=interaction(car, day, donor), fill=car)) + 
  geom_point(aes(color=car)) + 
  facet_grid(t_type+donor ~ .) + 
  ggtitle("CD4 - Incucyte K562 Killing by mKate Total Integrated Intensity (normalized to starting) at 24 Hours") +
  theme_minimal(base_size=20) +
  theme(plot.title = element_text(size = 25)) + 
  scale_y_continuous('mKate Total Integrated Intensity\n(normalized to growth without CAR)',
    limits=c(1e-2,1e1), trans=log10_trans()) +
