...
bb28z_dt <- incucyte_dt[car %in% c('41BB','CD28','zeta') & k562 != 'none' & noise==F]
...
k562_thresh <- function(incucyte_dt, pct_rem){
  ...
  cross_dt <- incucyte_dt[, list(
      t_cross = Elapsed[which.min(abs(mean_value_none-(pct_rem/100)))],
  ...
  cd4_thresh <- ggplot(incucyte_dt[t_type=='cd4' & measurement==chosen_measure &
    ...
    geom_ribbon(aes(x=Elapsed, 
                    ymin=mean_value_none-std_err_value_none_log, 
                    ymax=mean_value_none+std_err_value_none_log, 
                    fill=as.factor(donor)), alpha=0.3) +
