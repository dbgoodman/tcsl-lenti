...
dendrogram <- function(umap_fcs_dt, cell_type) {
  ...
  param_cluster_pct_dt$variable <- factor(
   ...
   #annotate clusters as per variable values (removed gfp, myc)
  '%notin%' <- Negate('%in%')
  annotations <- param_cluster_pct_dt %>%
  mutate(variable=gsub("\\_t$", "", variable)) %>%
  filter(variable %notin% c('gfp','myc')) %>%
  ...
  grid.arrange(dendro_plot, tbl, nrow = 2, as.table = TRUE)
} 

#cd8
dendrogram(umap_cd8_fcs_dt, 'cd8')

#cd4
dendrogram(umap_cd4_fcs_dt, 'cd4')
```
...
day_cluster <- function(umap_fcs_dt, cell_type) {
...
#cd8
day_cluster(umap_cd8_fcs_dt, 'CD8')

#cd4
day_cluster(umap_cd4_fcs_dt, 'CD4')
```

### Percent of cells in cluster, by day

```{r, echo=F, fig.width=15, fig.height=8}
day_cluster_total <- function(umap_fcs_dt, cell_type) {
...
#cd8
day_cluster_total(umap_cd8_fcs_dt, 'CD8')

#cd4
day_cluster_total(umap_cd4_fcs_dt, 'CD4')
...
car_per_day_per_cluster <- function(umap_fcs_dt, cell_type) {
...
#cd8
car_per_day_per_cluster(umap_cd8_fcs_dt, "CD8")

#cd4
car_per_day_per_cluster(umap_cd4_fcs_dt, "CD4")
```
##### summed across rows
```{r, echo=F, fig.width=15, fig.height=8}
car_per_day_per_cluster_total <- function(umap_fcs_dt, cell_type) {
...
#cd8
car_per_day_per_cluster_total(umap_cd8_fcs_dt, "CD8")

#cd4
car_per_day_per_cluster_total(umap_cd4_fcs_dt, "CD4")
```

#### Bar charts showing how each CAR changes cluster by day
```{r fig.width=15, fig.height=8}
car_clusters_by_day <- function(umap_fcs_dt, cell_type) {
...
#cd8
car_clusters_by_day(umap_cd8_fcs_dt, "CD8")

#cd4
car_clusters_by_day(umap_cd4_fcs_dt, "CD4")
```

#### Bar charts showing distribution of each CAR in clusters per day 
```{r fig.width=15, fig.height=8}
car_clusters_by_square <- function(umap_fcs_dt, cell_type) {
  car_cluster_day_indiv_k562_total_pct_dt <- map_day_0(umap_fcs_dt)[,
      data.table(table(car,cluster,k562,t_type,day))][,
        list(cluster, pct=N/sum(N)), by=c('car','k562','t_type','day')][,
          pct_delta := scale(pct), by=c('car','k562','t_type','day')]
...
#cd8
car_clusters_by_square(umap_cd8_fcs_dt, "CD8")

#cd4
car_clusters_by_square(umap_cd4_fcs_dt, "CD4")
