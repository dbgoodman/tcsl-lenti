...
umap_out <- umap(umap_dt_in, min_dist=chosen_dist,
    n_neighbors=chosen_n_neighbor, learning_rate=0.5, verbose=T, n_threads=8, n_trees=100, n_sgd_threads=1,
    ret_nn=T)
...
ggdraw(plot_dendroheat_cd19) + draw_label('CD19+', x=0.9,y=0.9)
...
ggdraw(plot_dendroheat_cd19n) + draw_label('CD19-', x=0.9,y=0.9)
...
car_cluster_day_indiv_k562_pct_dt %>% head(100)
...
car_cluster_day_indiv_k562_total_pct_dt %>% head(100)
...
# data exploration
car_cluster_day_indiv_k562_bar_pct_dt %>% 
  filter(k562=='cd19+', t_type=='cd8', car=='zeta') %>% arrange(day) %>% head(100)
  
#filter for CD4+, CD19+, only CARs > 10% in a cluster in a given day
car_cluster_day_indiv_k562_bar_pct_dt %>% 
  filter(k562=='cd19+', t_type=='cd4', pct > 0.10) %>%
  ggplot() + geom_bar(aes(x=cluster, y=pct), color='black', stat='identity') +
    facet_grid(car~day, scales='free_x', space='free_x') +
    labs(title='CD4 CAR enrichment (>10%)\nper cluster per day', y='CAR',
      subtitle='CAR enrichment in cluster\n(absolute %) (summed across rows by cluster number)')

#filter for CD8+, CD19+, only CARs > 10% in a cluster in a given day
car_cluster_day_indiv_k562_bar_pct_dt %>% 
  filter(k562=='cd19+', t_type=='cd8', pct > 0.10) %>%
  ggplot() + geom_bar(aes(x=cluster, y=pct), color='black', stat='identity') +
    facet_grid(car~day, scales='free_x', space='free_x') +
...
#filter for CD8+, CD19+, only CARs > 10% in a cluster in a given day
car_cluster_day_indiv_k562_total_pct_dt %>% 
  filter(k562=='cd19+', t_type=='cd8', pct>.10) %>%
  ggplot() + geom_bar(aes(x=cluster, y=pct), color='black', stat='identity') +
    facet_grid(car~day, scales='free_x', space='free_x') +
    labs(title='CD8 CAR enrichment (>10%)\nper day per cluster', y='CAR',
      subtitle='CAR enrichment in cluster\n(z-score) (summed across CAR x day (each square))')
