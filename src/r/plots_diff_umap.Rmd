...
```{r}
# test kitchen
test_kitchen <- data.frame('variable' = rep('ccr7_t',6), cluster = c(10,4,5,13,7,1), clust_mean=c(1.9958137, -0.6116742, -0.4279137, -0.3149753, -0.6173423, -0.1729749))
...
test_kitchen %>%
  ...
  mutate(cluster = ifelse(clust_mean > 0, paste(cluster,'+'), paste(cluster,'-'))) %>%
  summarise(cluster = paste(cluster, collapse=", ")) %>%
  unite(variable, variable, cluster, sep=" : ", remove=F) %>%
  ungroup() %>%
  pull(variable) %>% rep(10) 

# replace var with cluster
test_kitchen %>%
  ...
  mutate(variable = ifelse(clust_mean > 0, paste(variable,'+'), paste(variable,'-'))) %>%
  summarise(variable = paste(variable, collapse=", ")) %>%
  unite(cluster, cluster, variable, sep=" : ", remove=F) %>%
  select(cluster)
...
dendrogram <- function(umap_fcs_dt, cell_type) {
  ...
  param_cluster_pct_dt$variable <- factor(
   ...
   #annotate clusters as per variable values (remove gfp, myc first)
