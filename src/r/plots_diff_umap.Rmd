...
## Remove Day 33, KLRG1, untrans
```{r, eval=redo_umap_clustering}
diff_dt_cd4 <- diff_dt_cd4 %>% filter(!day == 33) %>% filter(!car %in% c('KLRG1', 'untrans'))

diff_dt_cd8 <- diff_dt_cd8 %>% filter(!day == 33) %>% filter(!car %in% c('KLRG1', 'untrans'))
```
