...
map_day_0 <- function(df, keep_none=T) {
  return(rbind(
    df[k562!='none'],
    df[rep(keep_none, .N) && k562=='none'],
...
#filter diff_dt by cd4/8 type, cd19+, donor==1
diff_dt_cd4 <- diff_dt %>% filter(t_type == 'cd4', k562 %in% c('none', 'cd19+'), donor==1)
diff_dt_cd8 <- diff_dt %>% filter(t_type == 'cd8', k562 %in% c('none', 'cd19+'), donor==1)
...
visualize_days_umap <- function(df) {
  ...
  ggplot(df, aes(x=umap_1, y=umap_2, color=as.factor(day)))+geom_point(size=0.1,alpha = 0.5, position=position_jitter(h=0.15,w=0.15))+theme_bw()+theme(panel.grid.major = element_blank(), panel.grid.minor=element_blank())+geom_label_repel(aes(label=day), data=label_days_umap)+guides(colour=FALSE)+ggtitle('UMAP by Day') + scale_color_viridis(discrete=TRUE)
}
...
## Contour plots - visualize CARS by day, cluster
...
all_plots <- function(umap_fcs_dt, umap_cluster_dt, cell_type, dendro=T) {
  ...
  umap_heatmap <- umap_marker_heatmap(umap_fcs_dt)
...
#### Identify CD8 cluster cell types
```{r}
# manually determined 
...
#### Identify CD4 cluster cell types
```{r}
