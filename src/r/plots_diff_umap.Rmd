...
lin_cd8 <- getLineages(rd_matrix_cd8, clusterLabels_cd8, start.clus = '14')
...
crv_cd8 <- getCurves(lin_cd8, approx_points=100)

# save the trajectory inference data
save(crv_cd8, file=file.path(here::here('..','data','diff.sampled.umap_cd8_trajectory.Rdata')))
...
lines(crv_cd8, lwd = 2, type = 'lineages', col = 'black', show.constraints = TRUE)
```

```{r}
# plot psuedotime values for each lineage (not ready)
nc <- 3
pt <- slingPseudotime(crv_cd8)
nms <- colnames(pt)
...
for (i in nms) {
  ...
  plot(reducedDim(lin_cd8), col = colors, pch = 16, cex = 0.5, main = i)
  lines(crv_cd8, lwd = 2, col = 'black', type = 'lineages')
}
```
...
```{r}
umap_cd8_fcs_pseudotime_dt <- cbind(umap_cd8_fcs_dt, slingPseudotime(crv_cd8) %>% as.data.frame())
