...
growth_summ_dt <- data.table(growth_dt)[, {
...
plot_grid(
    ...
    ggplot(
        ...
        geom_boxplot() + 
        geom_point(position = position_jitterdodge()) + theme_bw(),
    ncol=2, labels=c('Area=LWD','Area=LWW'))
...
plot_grid(
    ...
    ggplot(
        ...
        theme_bw(), 
    scale=0.9,
    ncol=2, nrow=1, labels=c('Area=LWD','Area=LWW'))
...
plot_grid(
    ...
    ggplot(
        ...
        theme_bw(), 
    scale=0.9,
    ncol=2, nrow=1, labels=c('Area, vs d11=LWD','Area, vs d11=LWW'))

# ggplot(growth_dt) + 
#     geom_line(aes(y=log(Area2), x=day, color=car, group=interaction(mouse, cage))) + 
#     facet_wrap(.~car) + 
#     geom_ribbon(data=growth_summ_dt, aes(x=day, ymin=lo, ymax=hi, color=car, fill=car), 
#     alpha=0.2)
```