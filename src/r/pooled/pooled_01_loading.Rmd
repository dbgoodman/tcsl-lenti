...
# map back to actual names from costim table - we want to rename some
# use correct names, from costim table
csv.dir <- here::here(
  '..','..',
  's3-roybal-tcsl',
  'lenti_screen_compiled_data','data','csv')

# names of domains to map onto actual car names
domain_names <- fread(file.path(csv.dir, 'costim_table.csv'), fill=T)[,
  list(CAR.orig=r_name, CAR.new=real_name)]

read.counts <- merge(read.counts, domain_names,
  by.x='CAR.align', by.y='CAR.orig', all=T)

#rename CARs, save old name as CAR.orig
read.counts[, CAR.orig := CAR.align]
read.counts[, CAR.align := CAR.new]
...
read.counts <- read.counts[!(CAR.orig %in% remove.cars)]
...
nonaligning.reads.plot <- ggplot(read.counts[, 
    sum(counts[CAR.align == '*'])/sum(counts),
    by=.(batch, donor, timepoint, assay, t.type, k.type, bin)]) + 
