...
plot_all_reps <- function(value_col) {
  ...
  all_rep_comparisons <- all_rep_comparisons[, 
    cast_comparison(read.counts, x.group, y.group, value_col=value_col)[, 
  ...
  all_rep_comparisons[, rep_pair := paste(gsub('(prolif\\d_d\\d).*','\\1', x.group), gsub('(prolif\\d_d\\d).*','\\1', y.group), sep='|')]
  all_rep_comparisons[, assay_kt := paste(assay,t.type,k.type, sep='|')]
  
  return(ggplot(all_rep_comparisons) + 
    geom_point(aes(x_scaled_sep, y_scaled_sep, fill=rel.baseline.log, 
      label=CAR.align), shape = 21, colour = "grey30") + 
...
comp.df <- copy(read.counts[assay != 'baseline'])
...
plot_measure_pair <- function(measure_i, measure_j) {
  return(ggplot(comp.df[batch != 'post-cytof']) + 
    geom_point(aes_string(x=measure_i, y=measure_j, 
      fill='rel.baseline.log',
      label='CAR.align'),
      shape = 21, colour = "grey30") + 
    scale_fill_distiller(palette='BrBG', 
      limit=c(-1,1) * max(abs(comp.df$rel.baseline.log))) +
    facet_grid(batch+donor ~ k.type + t.type + assay))
}

plot_measure_pair('CAR.score.scaled', 'car.abund.rel.baseline.scaled')
plot_measure_pair('CAR.score.scaled', 'car.abund.rel.prev.scaled')
plot_measure_pair('car.abund.rel.baseline.scaled', 'car.abund.rel.prev.scaled')
```
