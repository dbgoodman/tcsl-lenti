...
barplot_stacked_plot_horiz <- ggplot(subset_barplot_reads) +
  ...
  geom_point(aes(x=CAR.ranked.sep, y=-0.0175, color=CAR.align),
      pch=17,
      size=2.8) +
  scale_color_manual(values=c(other_car_colors, car_colors, '4-1BB'=car_colors[['41BB']]), guide='none')
...
fig2_abundance_over_time_vert <- ggplot(abund_plot_data[rev(order(CAR.topbot))], aes(
    y=mean.car.abund.rel.baseline, x=assay,
    color=CAR.topbot,
    group=CAR.align,
    label=CAR.align)) + 
...
prolif_4v8_rel_vert <- ggplot(cast_4v8_abund[
  ...
  theme_minimal() + 
  labs(y='log2 CD8:CD4 ratio', 
       x='Mean Proliferation or Expansion, log2 mean FC') +
  theme(panel.border=element_rect(fill=NA)) +
  guides(size=F)
