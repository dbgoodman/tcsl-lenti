...
subset_barplot_reads <- read.counts[
  assay == 'CTV1' & batch == 'prolif2' & donor == 'd2' & k.type == 'pos']
...
barplot_stacked_plot_horiz <- ggplot(subset_barplot_reads) +
  ...
  scale_y_continuous(limits = c(0,1), expand = expansion(mult = c(0, 0)), 
                     breaks=c(0, .5, 1), labels=percent) +
  theme_bw(base_size=12) + theme(axis.ticks = element_blank()) + 
  theme(
    axis.text.x = element_text(size=9, angle = 90, vjust = 0.5, hjust=1),
    panel.grid.major = element_blank(),
    panel.grid.minor = element_blank(),
...
barplot_stacked_multi_plot <- ggplot(subset_barplot_reads) +
  ...
  theme(
    ...
    panel.background = element_rect(fill= brewer.pal(4, 'YlGn')[1]),
...
fig_2a_single_dendro <- make_dendroheatmap(
  ...
  legend_theme=theme(
      ...
      plot.margin = unit(c(5.5,66,5.5,44), "pt")))
...
fig2_abundance_over_time_vert <- ggplot(prolif_data[order(CAR.type)], aes(
    y=mean.car.abund.rel.baseline, x=assay,
    color=CAR.type,
    group=CAR.align,
    label=CAR.align)) + 
...
cytokine_panel <- ggplot(cytokine.pct.pos.melt) + 
...
```{r}
plot_a <- plot_grid(barplot_stacked_plot_horiz+theme(legend.position = "none"), barplot_stacked_multi_plot,
  rel_widths=c(3.5,1),
  align='h',
  ncol = 2)
...
plot_abcde <- plot_grid(
  ...
  plot_e,
  labels=c('A','B','','E'), ncol=1, nrow=4, rel_heights=c(1,0.8,1.2,1))
