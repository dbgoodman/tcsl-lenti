...
make_dendroheatmap <- function(
  df, title, dendro_units=grid::unit(0.2, "null"), legend=T)
{
  ...
  if (!legend) {
    var_heatmap <- var_heatmap + theme(legend.position = "none") 
  }
...
fig_2a_single_dendro <- make_dendroheatmap(chosen_metrics, title='')
...
fig2_abundance_over_time <- ggplot(prolif_data[order(CAR.type)], aes(
  ...
  facet_grid(. ~ t.type) +
  scale_x_discrete(
    'Days after initial stimulation', 
    expand = c(0, 0.4, 0, 1), 
    labels=c('Baseline', 'Day 3-4', 'Day 14-16', 'Day 24')) +
  scale_y_continuous('log2 fold expansion') +
  theme_minimal() +
  theme(panel.border=element_rect(fill=NA))
...
cytokine_panel <- ggplot(cytokine.pct.pos[assay %in% c("IL2","IFNy")]) + 
  ...
  geom_point(
    ...
    size=2.5) + 
  scale_color_manual(
      values=c('CD19+'=cd19_val[2], 'CD19-'=cd19_val[1])) + 
  facet_wrap(~ assay) +
...
plot_bcd <- plot_grid(
  ...
  interbin_volcano,
  prolif_4v8 + coord_fixed(ratio=1),
  rel_heights=c(1.6, 2.3, 1.3),
  rel_widths=c(1,1,1),
  labels = c("B", "C", "D"), align='v', axis='lr',
  ncol = 1)

plot_abcd <- plot_grid(fig_2a_single_dendro, plot_bcd, labels=c('A',''))

plot_abcde <- plot_grid(plot_abcd, cytokine_panel, ncol=1,
  rel_heights=c(1,0.3), labels=c('','E'), align='v', axis='r')
```
