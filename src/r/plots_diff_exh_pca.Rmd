...
run_pca <- function(vars_dt, markers_list, 
  ...
  rotation_dt <- data.table(row.names(pca$rotation))
  setnames(rotation_dt, "V1", "input_var")
  rotation_dt <- cbind(rotation_dt, data.table(pca$rotation))
  #add back original vars 
  pca <- cbind(vars_cast_dt, data.table(pca$x))
  
  newlist <- list(pca, rotation_dt)
...
# diff = 'cd62l_t', 'cd27_t', 'ccr7_t', 'cd45ro_t', 'cd45ra_t', 'cd95_t'
# exh = 'tim3_t', 'lag3_t', 'pd1_t', 'cd39_t'
...
means_dt <- map_day_0(means_dt)

tot_pca <- run_pca(means_dt, markers_list)
pos_pca <- run_pca(means_dt[k562=='cd19+'], markers_list, target='cd19+')
neg_pca <- run_pca(means_dt[k562=='cd19-'], markers_list, target='cd19-')
cd4_pca <- run_pca(means_dt[k562=='cd19+' & t_type=='cd4'], markers_list, 
                   target='cd19+', tcell='cd4')
cd8_pca <- run_pca(means_dt[k562=='cd19+' & t_type=='cd8'], markers_list, 
                   target='cd19+', tcell='cd8')
        
ggplot(run_pca(means_dt, markers_list)[[1]], aes(x=PC1, y=PC2)) + geom_point()
```
...
a <- ggplot(tot_pca[[1]], aes(x=PC1, y=PC2)) + 
  geom_point(aes(color=car)) +
  labs(title='general PCA')
b <- ggplot(pos_pca[[1]], aes(x=PC1, y=PC2)) + 
  geom_point(aes(color=car)) +
  labs(title='CD19+ only PCA')
c <- ggplot(neg_pca[[1]], aes(x=PC1, y=PC2)) + 
  geom_point(aes(color=car)) +
  labs(title='CD19- only PCA')
d <- ggplot(cd4_pca[[1]], aes(x=PC1, y=PC2)) + 
  geom_point(aes(color=car)) +
  labs(title='CD4 only PCA')
e <- ggplot(cd8_pca[[1]], aes(x=PC1, y=PC2)) + 
  geom_point(aes(color=car)) +
...
p <- ggplot(pos_pca[[1]], aes(x=PC1, y=PC2, text=paste("Costim:", car, "<br>Day:", day, "<br>K562:", k562))) +
...
ggplot() +
  geom_bar(data=melt(cd8_pca[[2]], id.vars='input_var', variable.name='PC', value.name='value'),
...
ggplot(cd8_pca[[2]], aes(x=reorder(input_var, PC1), y=PC1)) +
...
tot_seg_dt <- arr_by_day(tot_pca[[1]])
cd4_seg_dt <- arr_by_day(cd4_pca[[1]])
cd8_seg_dt <- arr_by_day(cd8_pca[[1]])
pos_seg_dt <- arr_by_day(pos_pca[[1]])
neg_seg_dt <- arr_by_day(neg_pca[[1]])
...
ggplot(cd8_pca[[1]], aes(x=PC1, y=PC2)) + 
...
cd8_arr <- ggplot(cd8_pca[[1]], aes(x=PC1, y=PC2)) + 
  geom_point(data=cd8_pca[[1]][day==0], aes(color=car, size=1)) +
  geom_segment(data=cd8_seg_dt, 
...
ggsave(paste0(output_dir, '/CD8_pos_PCA_d0_d33.pdf'),
       cd8_arr, width = 7, height = 4,
       units = 'in', dpi = 300)



cd4_arr <- ggplot(cd4_pca[[1]], aes(x=PC1, y=PC2)) + 
  geom_point(data=cd4_pca[[1]][day==0], aes(color=car, size=1)) +
  geom_segment(data=cd4_seg_dt, 
               aes(x=PC1_a, y=PC2_a, xend=PC1_b, yend=PC2_b, color=car, alpha=0.1),
               size=2,
               arrow=arrow(length=unit(0.3,"cm"), type="closed")) +
  scale_color_brewer(palette="Paired") +
  labs(title="CD4 CD19+ only PCA - diff") +
  theme_bw(base_size=9) +
  theme(aspect.ratio=1, 
        plot.title = element_text(hjust = 0.5)) +
  guides(alpha=F, size=F)
...
ggsave(paste0(output_dir, '/CD4_pos_PCA_d0_d33.pdf'),
       cd4_arr, width = 7, height = 4,
       units = 'in', dpi = 300)
...
pos_arr <- ggplot(pos_pca[[1]], aes(x=PC1, y=PC2)) + 
  geom_point(data=pos_pca[[1]][day==0], aes(color=car, size=1, shape=t_type)) +
  geom_segment(data=pos_seg_dt, 
               aes(x=PC1_a, y=PC2_a, xend=PC1_b, yend=PC2_b, color=car, alpha=0.1),
               arrow=arrow(length=unit(0.3,"cm"), type="closed")) +
  scale_color_igv() +
  labs(title="CD19+ only PCA") +
...
neg_arr <- ggplot(neg_pca[[1]], aes(x=PC1, y=PC2)) + 
  geom_point(data=neg_pca[[1]][day==0], aes(color=car, size=1, shape=t_type)) +
  geom_segment(data=neg_seg_dt, 
               aes(x=PC1_a, y=PC2_a, xend=PC1_b, yend=PC2_b, color=car, alpha=0.1),
               arrow=arrow(length=unit(0.3,"cm"), type="closed")) +
  scale_color_igv() +
  labs(title="CD19- only PCA") +
