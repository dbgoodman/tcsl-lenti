...
# RAW DATA TABLE - INCUCYTE 

incucyte_export <- rbind(
  incucyte_new_dt[
    k562=='mkate_cd19' & !noise & t_type == 'CD4' & 
    measurement==chosen_measure & car!='none'],
  incucyte_new_dt[
...
incucyte_export_rows <- list(
  car='Domain Name',
  day='Measurement Day',
  well='Measurement Well',
  rep='Measurement Replicate',
  Elapsed='Time Elapsed',
  donor='Donor Number',
  t_type='T Cell Type',
  k562='K562 Condition',
  mean_value_none='Relative tumor growth vs. no T cells, raw image intensity',
  mean_value_none_rollmean10_rmnoise='10-sample rolling mean of raw image intensity for relative tumor growth vs. no T cells',
  mean_value_none_rollmean10='10-sample rolling mean of relative tumor growth vs. no T cells, (with noisy/failed images kept)')

incucyte_export <- incucyte_export[, 
  names(incucyte_export_rows), with=F]

names(incucyte_export) <- unlist(incucyte_export_rows)

fwrite(list('# Incucyte arrayed data: Fig 5D, 5E, S5C, S5D, S5E'),
    here::here('..','tables','fig5d_incucyte.csv'))
fwrite(incucyte_export, append=T, col.names=T,
    here::here('..','tables','fig5d_incucyte.csv'))
...
# RAW DATA TABLE - CYTOKINES ARRAYED

flow.means.dt_rows <- list(
  car='Domain Name',
  days='Measurement Day',
  cytokine='Measured Cytokine',
  donor='Donor Number',
  cell.type='T Cell Type',
  k562='K562 Condition',
  pct.mean='Mean percentage of Cytokine-positive cells',
  pct.stdev='Stdev. of percentage of Cytokine-positive cells',
  pct.mean.donor='Between-donor mean percentage of Cytokine-positive cells')

flow.means.dt_export <- copy(flow.means.dt[, 
  names(flow.means.dt_rows), with=F])

names(flow.means.dt_export) <- unlist(flow.means.dt_rows)

fwrite(list('# CTV arrayed data: Fig 4B'),
    here::here('..','tables','fig5c_cytokines.csv'))
fwrite(flow.means.dt_export, append=T, col.names=T,
    here::here('..','tables','fig5c_cytokines.csv'))
...
# RAW DATA TABLE - REPORTERS 

plot.signaling.dt_rows <- list(
  car_lab='Domain Name',
  stim_lab='Stimulation Condition',
  Hours='Hours after stimulation',
  combo_mcherry_pct_v_unt='Percent of reporter-transduced cells cells relative to no CAR',
  combo_mcherry_percent='Percent of reporter-transduced cells cells transduced with reporter',
  combo_mcherry_mfi='MFI of reporter-transduced cells cells',
  combo_mcherry_mfi_v_unt='MFI of reporter-transduced cells relative to no CAR',
  line_lab='Jurkat Reporter Line')

plot.signaling.dt_export <- plot.signaling.dt[, 
  names(plot.signaling.dt_rows), with=F]

names(plot.signaling.dt_export) <- unlist(plot.signaling.dt_rows)

fwrite(list('# Reporter Line arrayed data: Fig 5G, S5G'),
    here::here('..','tables','fig5g_reporters.csv'))
fwrite(plot.signaling.dt_export, append=T, col.names=T,
    here::here('..','tables','fig5g_reporters.csv'))
...
s5e <- plot_grid(
  plots$combined_sig_plot_nomut_pos_ap1 + labs(subtitle='AP1') + s5e_ggtheme,
  plots$combined_sig_plot_nomut_pos_nfat + labs(subtitle='NFAT') + s5e_ggtheme
)
