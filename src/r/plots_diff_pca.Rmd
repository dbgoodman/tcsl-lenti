...
diff_pca <- prcomp(diff_cast_dt[,-c(1:4)], scale. = T, center = T)
...
neg_pca <- prcomp(neg_cast_dt[,-c(1:4)], scale. = T, center = T)
pos_pca <- prcomp(pos_cast_dt[,-c(1:4)], scale. = T, center = T)
cd4_pca <- prcomp(cd4_cast_dt[,-c(1:4)], scale. = T, center = T)
cd8_pca <- prcomp(cd8_cast_dt[,-c(1:4)], scale. = T, center = T)
...
cd4_pc1_dt <- cd4_rot_dt[, 1:2]
setnames(cd4_pc1_dt, "PC1", "value")
cd4_pc1_dt[, PC := 1][order(-abs(value)), ]

cd4_pc2_dt <- cd4_rot_dt[, c(1,3)]
setnames(cd4_pc2_dt, "PC2", "value")
cd4_pc2_dt[, PC := 2][order(-abs(value)), ]

cd4_pc_dt <- rbind(cd4_pc1_dt, cd4_pc2_dt)[order(-abs(value), PC)]

cd8_pc1_dt <- cd8_rot_dt[, 1:2]
setnames(cd8_pc1_dt, "PC1", "value")
cd8_pc1_dt[, PC := 1][order(-abs(value)), ]

cd8_pc2_dt <- cd8_rot_dt[, c(1,3)]
setnames(cd8_pc2_dt, "PC2", "value")
cd8_pc2_dt[, PC := 2][order(-abs(value)), ]

cd8_pc_dt <- rbind(cd8_pc1_dt, cd8_pc2_dt)[order(-abs(value), PC)]
...
cd4_pc_plot <- ggplot(cd4_pc_dt[1:6,], aes(x=V1, y=value)) +
  geom_bar(stat="identity") +
...
cd8_pc_plot <- ggplot(cd8_pc_dt[1:6,], aes(x=V1, y=value)) +
  geom_bar(stat="identity") +
  facet_grid(. ~ PC, scales="free") +
  geom_hline(aes(yintercept=0)) +
  theme_bw(base_size=16) +
