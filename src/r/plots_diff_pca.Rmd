...
markers_list <- c('cd62l_t', 'cd27_t', 'ccr7_t', 'cd45ro_t', 'cd45ra_t', 'cd95_5', 'myc_t', 'tim3_t', 'gfp_t', 'lag3_t', 'pd1_t', 'cd39_t')
...
diff_means_dt <- diff_dt[!is.na(day) & !is.na(donor) & !is.na(value) & variable %in% markers_list & car!='untrans', 
  list(m= mean(value, na.rm=T), sd= sd(value, na.rm=T), n= .N), 
  by=c('day','donor','car','k562','t_type','variable')]

exh_means_dt <- exh_dt[!is.na(day) & !is.na(donor) & !is.na(value) & variable %in% markers_list & car!='untrans', 
  list(m= mean(value, na.rm=T), sd= sd(value, na.rm=T), n= .N), 
  by=c('day','donor','car','k562','t_type','variable')]
...
diff_cast_dt <- dcast(diff_means_dt[!is.na(m) & n > 20], 
                      ...
                      value.var=c('m'))
setnames(diff_cast_dt, "myc_t", "diff_myc_t")

exh_cast_dt <- dcast(exh_means_dt[!is.na(m) & n > 20],
                     day + donor + car + k562 + t_type ~ variable, 
                     value.var=c('m'))
setnames(exh_cast_dt, "myc_t", "exh_myc_t")


# use function to make day 0 k562 as cd19+ and cd19- instead of none
diff_cast_dt <- map_day_0(diff_cast_dt)
exh_cast_dt <- map_day_0(exh_cast_dt)

exp_cast_dt <- merge(diff_cast_dt, exh_cast_dt, 
                     by=c("donor","day","car","k562","t_type"))


# getting pca for all, cd19- only, cd19+ only, cd4 cd19+ only, cd8 cd19+ only with sd
# make sure the -c(1:4) or -c(1:5) is correct for the table
define_pca <- function(df, target=c('cd19+','cd19-'), tcell=c('cd4','cd8')) {
  return(prcomp(df[k562 %in% target & t_type %in% tcell][,-c(1:5)], scale. = T, center = T))
}

tot_pca <- define_pca(exp_cast_dt)
neg_pca <- define_pca(exp_cast_dt, target='cd19-')
pos_pca <- define_pca(exp_cast_dt, target='cd19+')
cd4_pca <- define_pca(exp_cast_dt, target='cd19+', tcell='cd4')
cd8_pca <- define_pca(exp_cast_dt, target='cd19+', tcell='cd8')

ggplot(data.table(tot_pca$x), aes(x=PC1, y=PC2)) + geom_point()
...
ggplot(pos_cbind_dt, aes(x=PC1, y=PC2)) +
  geom_point(aes(color=as.factor(day), shape=as.factor(donor), size=1)) +
  facet_grid(t_type ~ car)
