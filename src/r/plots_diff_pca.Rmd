...
arr_by_day <- function(df) {
  seg_1 <- df[day != 33][, c('day', 'car', 'k562', 't_type', 'PC1', 'PC2', 'PC3')]
  colnames(seg_1)[colnames(seg_1) %in% c("day", "PC1", "PC2", "PC3")] <- c("day_a", "PC1_a", "PC2_a", "PC3_a")
  seg_1[, day_b := diff_days[match(day_a, diff_days)+1]]
  seg_2 <- df[day != 0][, c('day', 'car', 'k562', 't_type', 'PC1', 'PC2', 'PC3')]
  colnames(seg_2)[colnames(seg_2) %in% c("day", "PC1", "PC2", "PC3")] <- c("day_b", "PC1_b", "PC2_b", "PC3_b")
